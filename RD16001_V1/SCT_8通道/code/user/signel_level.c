...
char timeLevel[50];
char CmdLevel[]={0x55,0xaa,0x00,0x0e,0x04,0x00,0x00};//水准仪的命令
unsigned int detectLevelTime = 0;
unsigned char measureLevelTimes = 0;
  ...
  while(1)
      {
        if(DtuStatu() == 1)
        {
            while(1)
            {
              if(GetLevel(CmdLevel,sensorLevel) == 1)
                  {
                    GetRtc(timeLevel);
                    printf("%s\r\n",timeLevel);
                    printf("level data: %s\r\n",sensorLevel);
                    memset(timeLevel,'\0',50);
        ...
        {
          detectLevelTime++;
          if(detectLevelTime == 30)
          {detectLevelTime = 0; printf("disconnect\r\n");break;}
          delay_ms(1000);
          printf("offline...\r\n");
        }
      }

...
#define SST25VF064_CS_LOW 			P2OUT &= ~BIT0
#define SST25VF064_CS_HIGH		 	P2OUT |= BIT0//P20

#define SST25VF0_CS_Low();                      P2OUT &= ~BIT0;
...
void ExternFlashPageProgram(long Dst, unsigned char *pupper,long amount)
{
	long i;
	i=0;	
	//WREN
	...
	if(amount>(256-(Dst&0xFF)))        //写入的长度超出页边界结束地址
	{
		...
		{	
			SST25VF064WriteReadByte( (*pupper++));		        // send byte to be programmed 
		}
		SST25VF0_CS_High();						              // disable device 

...
//串口接收数据完毕
extern unsigned char uart_recived_flg;//串口接收到数据的标志
extern unsigned char JN_recived_flg;//zigbee接收到response的标志
unsigned char first_return = 0;//zigbee的response的第一个回车

extern ZigbeeStatusType ZigbeeStatus;                 //刚开始是上电状态的
...
__interrupt void UART0_RX_ISR(void)
{
  ...
  /*接收到的数据存起来*/
  u8data = U0RXBUF;
  LoopRAMWriteByte(u8data);//添加到环形缓冲里面
  if(u8data == '\n')
  {
    uart_recived_flg = 1;
  }
...
#ifdef COMUNICATION_ZIGBEE 
  /**********************************ZIGBEE_MODE_NET****************************************/
  #ifdef ZIGBEE_MODE_NET
    ////////////////////////////ZIGBEE_TYPE_END////////////////////////////////
    #ifdef ZIGBEE_TYPE_END
      JNRAMWriteByte(u8data);//添加到环形缓冲里面
      //复位状态
      if(ZigbeeStatus == ZIGBEE_STATUS_RST)
      {
        if(u8data == '\n' && first_return == 0)
        {
          first_return = 1;
      ...
      //CFG状态
      if(ZigbeeStatus == ZIGBEE_STATUS_CFG)
      {
        if(u8data == '\n' && first_return == 0)
        {
          first_return = 1;
      ...
      //INI状态
      if(ZigbeeStatus == ZIGBEE_STATUS_INI)
      {
        if(u8data == '\n' && first_return == 0)
        {
          first_return = 1;
      ...
      //STR状态
      if(ZigbeeStatus == ZIGBEE_STATUS_STR)
      {
        if(u8data == '\n' && first_return == 0)
        {
          first_return = 1;
      ...
      //组网状态
      if(ZigbeeStatus == ZIGBEE_STATUS_NET)
      {
        if(u8data == '\n')
        {
          JN_recived_flg = 1;

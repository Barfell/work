...
unsigned char DTcmdExtract(char *dat, DTprotocolStructType *DTprotocolStructure)
{
	...
	while(*(dat+i) != ' ')
	{
		*(temp+j) = *(dat+i);
		...
		{memset(temp,'\0',DATA_MAX_LEGTH);return 0;}
	}
	/*-------------Header------------*/
	if(strncmp(temp, DT_CMD_HEAD, sizeof(DT_CMD_HEAD)) == 0)
	{
		...
		{ memset(temp,'\0',DATA_MAX_LEGTH);memset(DTprotocolStructure,0,sizeof(*DTprotocolStructure)); return 0;}
	}
  ...
  if(uart_recived_flg == 1)
      {
        uart_recived_flg = 0;
        //读取出环形缓冲的数据
        LoopRAMReadLength(Uartloop,strcmd123, Uartloop.LoopDatLength);
        ...
        {
          printf("%s is not a cmd ! ",strcmd123);
          memset(strcmd123,0,sizeof(strcmd123));
        }
      }
}

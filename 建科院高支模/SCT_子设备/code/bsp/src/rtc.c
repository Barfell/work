...
void GetRtc(char *rtcdata)
{
  ...
  sprintf(timestring,"20%s-%s-%s-%s:%s:%s|",stringyear,stringmounth,
                      stringdate,stringhour,stringmin,stringsec);
  memcpy(rtcdata,timestring,strlen(timestring));
  
}
void GetRtcHEX(char *rtcdata)
{
  char timestring[10];

  timestring[0] = (unsigned  char)RTCYEAR;
  timestring[1] = RTCMON;
  timestring[2] = RTCDAY;
  timestring[3] = RTCDOW;
  timestring[4] = RTCHOUR;
  timestring[5] = RTCMIN;
  timestring[6] = RTCSEC;
  
  memcpy(rtcdata,timestring,7);
}

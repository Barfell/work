...
*/
extern void My_SetPixelIndex(int uX, int uY, U8 uData);
void LCD_L0_SetPixelIndex(int x, int y, int PixelIndex) {
  ...
  {
    /* ... */
	  My_SetPixelIndex(x, y, PixelIndex);
  }
}
...
*   that no check on the parameters needs to be performed.
*/
extern unsigned My_GetPixelIndex(int uX, int uY);
unsigned int LCD_L0_GetPixelIndex(int x, int y) {
  ...
  {
    //PixelIndex = 0;/* ... */
	  PixelIndex = My_GetPixelIndex(x, y);
  }
  return PixelIndex;
}

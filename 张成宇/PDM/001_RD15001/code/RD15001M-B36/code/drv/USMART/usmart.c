...
STATIC USMART g_uSmart;
STATIC PUSMART pUsmart = &g_uSmart;
BOOL g_bDebug = FALSE;
STATIC INT UsmartTimerProc(POSTimer pOsTimer, PVOID pParam)
{
 	PauseOSTimer(&pUsmart->tm);
	if(g_bDebug == FALSE)
		PostEvent(1, HM_USMART); 
	else
		usmart_dev.scan();	//执行usmart扫描	
    ResetOSTimer(&pUsmart->tm);
    RunOSTimer(&pUsmart->tm);


    return SUCCESS;
}

STATIC VOID UsmartRegTimer(VOID)
{
	//注册定时器
	pUsmart->tm.Flag = OSTIMER_PAUSE;
	pUsmart->tm.Func = UsmartTimerProc;
	pUsmart->tm.Style = OSTIMER_LOOP;
	pUsmart->tm.Ticks = 100;
	RegisterOSTimer(&pUsmart->tm);
}

VOID UsmartHandle(VOID)
{
    usmart_dev.scan();	//执行usmart扫描	
}
...
void usmart_scan(void)
{
	...
	{					   
		...
		USART_RX_STA=0;//状态寄存器清空	 modified by syan： 强制清除，避免嵌套调用

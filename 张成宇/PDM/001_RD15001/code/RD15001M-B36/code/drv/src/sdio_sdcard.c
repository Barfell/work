...
#define SDIO_TEST_TIMEOUT   0XFFFFFF
...
__align(4) u8 SDIO_DATA_BUFFER[SD_UNIT_SIZE];						  
...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
	...
	}else if(CardType==SDIO_HIGH_CAPACITY_SD_CARD)	//高容量卡
	{
		...
		cardinfo->SD_csd.DeviceSize=(tmp&0x3F)<<16;//C_SIZE
 		...
 		tmp=(u8)((CSD_Tab[2]&0x0000FF00)>>8); 	//第10个字节	
 		cardinfo->CardCapacity=(long long)(cardinfo->SD_csd.DeviceSize+1)*SD_UNIT_SIZE*1024;//计算卡容量
		cardinfo->CardBlockSize=SD_UNIT_SIZE; 			//块大小固定为512字节
	}	  
...
u8 SD_ReadDisk(u8*buf,u32 sector,u8 cnt)
{
	...
	if((u32)buf%4!=0)
	{
	 	for(n=0;n<cnt;n++)
		{
		 	sta=SD_ReadBlock(SDIO_DATA_BUFFER,lsector+SD_UNIT_SIZE*n,SD_UNIT_SIZE);//单个sector的读操作
			memcpy(buf,SDIO_DATA_BUFFER,SD_UNIT_SIZE);
			buf+=SD_UNIT_SIZE;
		} 
	}else
	{
		if(cnt==1)sta=SD_ReadBlock(buf,lsector,SD_UNIT_SIZE);    	//单个sector的读操作
		else sta=SD_ReadMultiBlocks(buf,lsector,SD_UNIT_SIZE,cnt);//多个sector  
	}
	return sta;
}
//写SD卡
...
u8 SD_WriteDisk(u8*buf,u32 sector,u8 cnt)
{
    ...
    if((u32)buf%4!=0)
    {
        for(n=0;n<cnt;n++)
        {
            memcpy(SDIO_DATA_BUFFER,buf,SD_UNIT_SIZE);
            sta=SD_WriteBlock(SDIO_DATA_BUFFER,lsector+SD_UNIT_SIZE*n,SD_UNIT_SIZE);//单个sector的写操作
            buf+=SD_UNIT_SIZE;
        } 
    }
    else
    {
        if(cnt==1)
            sta=SD_WriteBlock(buf,lsector,SD_UNIT_SIZE);        //单个sector的写操作
        else 
            sta=SD_WriteMultiBlocks(buf,lsector,SD_UNIT_SIZE,cnt);  //多个sector  
    }

...
/* segment (from 000010H--000017H) in 39VF640X.       			*/
/*                                                                      */
...
int User_SecID_uint16_t_Program (uint16_t *Srcuint16_t, uint16_t *Dst, int length)
{
    ...
    while (index--)
    {
    	*sysAddress(0x555) = 0x00AA; // write data 0x00AA to device addr 0x5555
    	*sysAddress(0x2AA) = 0x0055; // write data 0x0055 to device addr 0x2AAA
    	*sysAddress(0x555) = 0x00A5; // write data 0x00A5 to device addr 0x5555
...
/* User Security ID segment, from 000010H--000017H, in 39VF640X.        */
/*                                                                      */
...
void User_SecID_Lock_Out (void)
{
    *sysAddress(0x555) = 0x00AA; // write data 0x00AA to device addr 0x5555
    *sysAddress(0x2AA) = 0x0055; // write data 0x0055 to device addr 0x2AAA
    *sysAddress(0x555) = 0x0085; // write data 0x0085 to device addr 0x5555
    *sysAddress(0x00F) = 0x0000; // write data 0x0000 to any addr
...
{
    *sysAddress(0x555) = 0x00B0; // write data 0x00B0 to any addr, i.e. 0x5555
...
/* operation that had been suspended in 39VF640X.  	 		*/
/*                                                                      */
/* Input:    None                                                       */
/*                                                                      */
/* Output:   None                                                       */
...
{
    *sysAddress(0x555) = 0x0030; // write data 0x0030 to any addr, i.e. 0x5555
}

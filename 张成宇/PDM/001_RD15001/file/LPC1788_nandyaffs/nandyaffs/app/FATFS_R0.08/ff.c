...
FRESULT f_mkfs (
	...
	BYTE sfd,		/* Partitioning rule 0:FDISK, 1:SFD     0 :volume start 
	sector = 0  1:volume start sector = 64 */
...
)
{
	...
	if (drv >= _DRIVES) return FR_INVALID_DRIVE;
	fs = FatFs[drv];
	if (!fs) return FR_NOT_ENABLED;// 已经f_mount()了
	fs->fs_type = 0;
	drv = LD2PD(drv);// 逻辑磁盘号对应的物理磁盘号
    ...
    //fdisk volume start sector is 63,sfd volume start sector is 0
	b_vol = (sfd == 1) ? 0 : 63;	/* Volume start sector */
	n_vol -= b_vol;                 /* n_vol -(volume start sector) */
    
	if (au & (au - 1)) au = 0;		/* Check validity of the allocation unit size */
    
	if (!au) {						/* AU auto selection */
		vs = n_vol / (2000 / (SS(fs) / 512)); 
        //vs = volume sector num/(2000/sector size/512)
	...
	if (sfd == 1) {
		...
		if (disk_write(drv, fs->win, 0, 1) != RES_OK)/* write sector 0*/
			return FR_DISK_ERR;
		md = 0xF8;
	}

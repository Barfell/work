...
int UHost_main()
{
	...
	SystemInit();                        
//	debug_frmwrk_init(); /* Initialize the serial port to view the log messages */
...
void  Main_Read (void)
{
    ...
    if (fdr > 0) {
        put_str("Reading from "); put_str(FILENAME_R);  put_str("\n");
        do {
      ...
      //  PRINT_Log("Read Complete\n");
	   put_str("Read Complete\n");
    } else {
       // PRINT_Log("Could not open file %s\n", FILENAME_R);
	    put_str("Could not open file "); put_str(FILENAME_R); put_str("\n");
        return;
    }
}
...
void  Main_Copy (void)
{
    ...
    if (fdr > 0) {
        fdw = FILE_Open(FILENAME_W, RDWR);
        if (fdw > 0) {
            put_str("Copying from");put_str(FILENAME_R); put_str( "to ");  put_str(FILENAME_W);put_str("\n");
            do {
                bytes_read = FILE_Read(fdr, UserBuffer, MAX_BUFFER_SIZE);
            ...
            put_str("Could not open file "); put_str(FILENAME_W); put_str("\n");
            return;
        }
        FILE_Close(fdr);
        put_str("Copy completed\n");
    } else {
        put_str("Could not open file "); put_str(FILENAME_R);	 put_str("\n");
        return;
    }

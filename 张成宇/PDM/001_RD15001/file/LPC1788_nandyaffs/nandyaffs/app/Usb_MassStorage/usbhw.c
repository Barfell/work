...
void USB_Init (void) {
  ...
  LPC_GPIO0->DIR = 0x00004000;		/* P0.14 defined as Outputs */
  LPC_GPIO0->CLR = ~(1<<14);		/* output 0 */
  ...
  LPC_USB->USBClkCtrl = 0x1B;                /* Dev, OTG, AHB clock enable */
  while ((LPC_USB->USBClkSt & 0x1B) != 0x1B);
...
{	uint32_t   int_status;
	...
	ie_status  = LPC_USB->InterruptEnable;
					...
					*/
					if (LPC_USB->RhPortStatus1 & OR_RH_PORT_CCS) {
						if (!gUSBConnected) {
							...
							put_str("Spurious status change (connected)?\n");
					} else {
						if (gUSBConnected) {
							...
							put_str("Spurious status change (disconnected)?\n");
					}
                }

...
int32_t LoadFromUart( int argc)
{
	...
	uint8_t * buf = ( uint8_t * ) 0x10008000UL;
	uint8_t RxTmp[8];

	if ( argc < 2 )
		download_addr = 0x10008000UL;

	buf = ( uint8_t * ) download_addr;

	_db_msg_((LPC_UART_TypeDef *)((LPC_UART_TypeDef *) ((0x40000000UL) + 0x0C000) ), "Now download file from uart0 to 0x%x...\n");

	i = 0;
	while ( i < 4 )
		RxTmp[i++] = _db_get_char((LPC_UART_TypeDef *)((LPC_UART_TypeDef *) ((0x40000000UL) + 0x0C000) ));

	i = 0;
	size = *( uint32_t * ) RxTmp - 4;
	while ( i < size )
		buf[i++] = _db_get_char((LPC_UART_TypeDef *)((LPC_UART_TypeDef *) ((0x40000000UL) + 0x0C000) ));
	...
	{
		_db_msg_((LPC_UART_TypeDef *)((LPC_UART_TypeDef *) ((0x40000000UL) + 0x0C000) ), "\nCheckSum error!\n");
		download_len = -1;
		return FALSE;
	}
	else {

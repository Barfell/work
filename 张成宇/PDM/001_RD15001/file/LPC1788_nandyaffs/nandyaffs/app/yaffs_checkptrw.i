...
static void yaffs_CheckpointFindNextCheckpointBlock(yaffs_Device *dev)
{
	...
	if(dev->blocksInCheckpoint < dev->checkpointMaxBlocks)
		for(i = dev->checkpointNextBlock; i <= dev->internalEndBlock; i++)
		{
			...
			dev->readChunkWithTagsFromNAND(dev,realignedChunk,0,&tags);
            
			do{ if((0x00008000) & (yaffs_traceMask | 0xF0000000)) ;} while(0);


			if(tags.sequenceNumber == 0x21){
...
int yaffs_CheckpointOpen(yaffs_Device *dev, int forWriting)
{
	...
	if(!dev->checkpointBuffer)
		dev->checkpointBuffer = yaffs_malloc(dev->totalBytesPerChunk);
	if(!dev->checkpointBuffer)
		return 0;
	...
	if(forWriting){
		...
		dev->checkpointBlockList = yaffs_malloc(sizeof(int) * dev->checkpointMaxBlocks);

...
#line 16 "..\\fs\\yaffsfs.c"
#line 17 "..\\fs\\yaffsfs.c"
#line 18 "..\\fs\\yaffsfs.c"
#line 19 "..\\fs\\yaffsfs.c"
...
static yaffsfs_Handle yaffsfs_handle[200];
...
static yaffs_Object *yaffsfs_FollowLink(yaffs_Object *obj,int symDepth)
{
	...
	while(obj && obj->variantType == YAFFS_OBJECT_TYPE_SYMLINK)
	{
		...
		if(yaffsfs_IsPathDivider(*alias))
		{
			...
			obj = yaffsfs_FindObject(0,alias,symDepth++);
		}
		else
		{
...
{
	int newHandle = -1;
	yaffsfs_Handle *oldPtr = 0;
	yaffsfs_Handle *newPtr = 0;
...
int yaffs_open(const char *path, int oflag, int mode)
{
	...
	if(handle >= 0)
	{
		...
		obj = yaffsfs_FindObject(0,path,0);
		...
		if(obj)
		{
			...
			if(((oflag & 0200) && alreadyOpen) || alreadyExclusive)
			{
				openDenied = 1;
			}

			
			if((oflag & 0200) && (oflag & 0100))
			{
				openDenied = 1;
				yaffsfs_SetError(-17);
				errorReported = 1;
		...
		else if((oflag & 0100))
		{
			...
			dir = yaffsfs_FindDirectory(0,path,&name,0);
			if(dir)
			{
		...
		if(obj && !openDenied)
		{
			h->obj = obj;
			h->inUse = 1;
	    	h->readOnly = (oflag & (01 | 02)) ? 0 : 1;
			h->append =  (oflag & 02000) ? 1 : 0;
			h->exclusive = (oflag & 0200) ? 1 : 0;
			h->position = 0;

                        obj->inUse++;
                        if((oflag & 01000) && !h->readOnly)
                        {
...
int yaffs_rename(const char *oldPath, const char *newPath)
{
	yaffs_Object *olddir = 0;
	yaffs_Object *newdir = 0;
	yaffs_Object *obj = 0;
	char *oldname;
	char *newname;
	int result= 0;
	...
	olddir = yaffsfs_FindDirectory(0,oldPath,&oldname,0);
	newdir = yaffsfs_FindDirectory(0,newPath,&newname,0);
	obj = yaffsfs_FindObject(0,oldPath,0);
	...
	else if(obj && obj->variantType == YAFFS_OBJECT_TYPE_DIRECTORY)
	{
		...
		if(!renameAllowed) yaffsfs_SetError(-13);
	}
...
static int yaffsfs_DoStat(yaffs_Object *obj,struct yaffs_stat *buf)
{
	...
	if(obj && buf)
	{
    	...
    	buf->st_mode = obj->yst_mode & ~0170000; 
...
int yaffs_access(const char *path, int amode)
{
	...
	if(obj)
	{
		...
		if((amode & 4) && !(obj->yst_mode & 0000400))
			access_ok = 0;
		if((amode & 2) && !(obj->yst_mode & 0000200))
			access_ok = 0;
		if((amode & 1) && !(obj->yst_mode & 0000100))
			access_ok = 0;
...
int yaffs_mkdir(const char *path, mode_t mode)
{
	yaffs_Object *parent = 0;
	yaffs_Object *dir = 0;
	char *name;
	int retVal= -1;

	yaffsfs_Lock();
	parent = yaffsfs_FindDirectory(0,path,&name,0);
	if(parent)
	...
	else
	{
		if(!parent){
			yaffsfs_SetError(-2); 
			_db_msg_((LPC_UART_TypeDef *)((LPC_UART_TypeDef *) ((0x40000000UL) + 0x0C000) ), "yaffs mkdir missing path");
		}
		else if (yaffs_FindObjectByName(parent,name)){
			yaffsfs_SetError(-17); 
			_db_msg_((LPC_UART_TypeDef *)((LPC_UART_TypeDef *) ((0x40000000UL) + 0x0C000) ), "yaffs mkdir already exists");
		}
		else
			yaffsfs_SetError(-28); 
		_db_msg_((LPC_UART_TypeDef *)((LPC_UART_TypeDef *) ((0x40000000UL) + 0x0C000) ), "yaffs mkdir no space");
		retVal = -1;
	...
	if(dsc &&
	   ...
	   dsc->dirObj->variantType == YAFFS_OBJECT_TYPE_DIRECTORY){
           ...
           if( ylist_empty(&dsc->dirObj->variant.directoryVariant.children)){
                ...
                dsc->nextReturn = ((yaffs_Object *)((char *)(dsc->dirObj->variant . directoryVariant . children . next)-(unsigned long)(&((yaffs_Object *)0)->siblings)));
               ...
               ylist_empty(&dsc->dirObj->variant.directoryVariant.children)){
                dsc->nextReturn = 0;
                   ...
                   if( next == &dsc->dirObj->variant.directoryVariant.children)
                        ...
                        dsc->nextReturn = ((yaffs_Object *)((char *)(next)-(unsigned long)(&((yaffs_Object *)0)->siblings)));
           }
        } else {
...
static void yaffsfs_RemoveObjectCallback(yaffs_Object *obj)
{
        ...
        if(!search_contexts.next)
         ...
         for (i = (&search_contexts)->next; i != (&search_contexts); i = i->next ) {
                if (i) {
                        dsc = ((yaffsfs_DirectorySearchContext *)((char *)(i)-(unsigned long)(&((yaffsfs_DirectorySearchContext *)0)->others)));
...
yaffs_DIR *yaffs_opendir(const char *dirname)
{
	yaffs_DIR *dir = 0;
 	yaffs_Object *obj = 0;
	yaffsfs_DirectorySearchContext *dsc = 0;

	yaffsfs_Lock();

	obj = yaffsfs_FindObject(0,dirname,0);

	if(obj && obj->variantType == YAFFS_OBJECT_TYPE_DIRECTORY)
	{

		dsc = yaffs_malloc(sizeof(yaffsfs_DirectorySearchContext));
		dir = (yaffs_DIR *)dsc;
		if(dsc)
		{
			memset(dsc,0,sizeof(yaffsfs_DirectorySearchContext));
                        dsc->magic = 0x5941FF53;
                        dsc->dirObj = obj;
                        strncpy(dsc->name,dirname,256);
                        do { (&dsc->others)->next = (&dsc->others); (&dsc->others)->prev = (&dsc->others); } while(0);

                        if(!search_contexts.next)
                                do { (&search_contexts)->next = (&search_contexts); (&search_contexts)->prev = (&search_contexts); } while(0);
...
struct yaffs_dirent *yaffs_readdir(yaffs_DIR *dirp)
{
	...
	struct yaffs_dirent *retVal = 0;

	yaffsfs_Lock();

	if(dsc && dsc->magic == 0x5941FF53){
		yaffsfs_SetError(0);
		if(dsc->nextReturn){
			dsc->de.d_ino = yaffs_GetEquivalentObject(dsc->nextReturn)->objectId;
			dsc->de.d_dont_use = (unsigned)dsc->nextReturn;
			dsc->de.d_off = dsc->offset++;
			yaffs_GetObjectName(dsc->nextReturn,dsc->de.d_name,256);
...
int yaffs_symlink(const char *oldpath, const char *newpath)
{
	...
	yaffsfs_Lock();
	parent = yaffsfs_FindDirectory(0,newpath,&name,0);
...
int yaffs_link(const char *oldpath, const char *newpath)
{
	...
	obj = yaffsfs_FindObject(0,oldpath,0);
	target = yaffsfs_FindObject(0,newpath,0);

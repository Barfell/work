...
STATIC U8 g_u8FlashType = SST25VF0_FLASH;
...
STATIC U32 g_u32AppAddress = STM_APP_ADDRESS;
...
STATIC VOID HandleCmdProc(U8 *pBuf, U16 u16Length)
{
	...
	if(u16Length == 7)
	{
		...
		else if((pBuf[0] == 0xfe) && (pBuf[1] == 0x08) && (pBuf[6] == 0xfe))//获取升级数据长度
		{
			...
			if(u16Loop != 0)
			{
				if(g_u8FlashType == SST25VF0_FLASH)
				{
					...
					{
						ExternFlashPageProgram(SST_APP_ADDRESS + g_u32WriteOffset + u32ReadOffset, szQueue + u32ReadOffset, SST_DATA_UNIT_SIZE);
					...
					ExternFlashPageProgram(SST_APP_ADDRESS + g_u32WriteOffset + u32ReadOffset, szQueue + u32ReadOffset, u16Loop - u32ReadOffset);
			...
			for(u32ReadOffset = 0; u32ReadOffset < g_u32WriteOffset; u32ReadOffset++)
			{
				if(g_u8FlashType == SST25VF0_FLASH)
				{
					ExternFlashHighSpeedRead(SST_APP_ADDRESS + u32ReadOffset, &u8Value, 1);
				}
				else
				{
		...
		else if((pBuf[0] == 0xfe) && (pBuf[1] == 0x0A) && (pBuf[6] == 0xfe))
		{
			u32ReadOffset = 0;
			
			while(u32ReadOffset < g_u32WriteOffset)
			{
				ExternFlashHighSpeedRead(SST_APP_ADDRESS + u32ReadOffset, szQueue, STM_PAGE_SIZE);
				
				FlashWriteSTM32(STM_APP_ADDRESS + u32ReadOffset, STM_PAGE_SIZE, szQueue);
				
				u32ReadOffset = u32ReadOffset + STM_PAGE_SIZE;
			}
			
			u32ReadOffset = u32ReadOffset - STM_PAGE_SIZE;
			
			ExternFlashHighSpeedRead(SST_APP_ADDRESS + u32ReadOffset, szQueue, g_u32WriteOffset - u32ReadOffset);
				
			FlashWriteSTM32(STM_APP_ADDRESS + u32ReadOffset, g_u32WriteOffset - u32ReadOffset, szQueue);
			...
			if(g_bUpdate == TRUE) 
			{
				...
				if(GetQueueLength(pDtuQueueInfo) >= STM_PAGE_SIZE)
				{
					...
					{					
						...
						while(u32ReadOffset < STM_PAGE_SIZE)
						{
							ExternFlashPageProgram(SST_APP_ADDRESS + g_u32WriteOffset + u32ReadOffset, szQueue + u32ReadOffset, SST_DATA_UNIT_SIZE);
						...
						ExternFlashPageProgram(SST_APP_ADDRESS + g_u32WriteOffset + u32ReadOffset, szQueue + u32ReadOffset, STM_PAGE_SIZE - u32ReadOffset);
				...
				else
				{
					...
					memset(szData, 0, sizeof(szData));
					sprintf((char *)szData, "boot receive = %d, write = %d\r\n, please wait\r\n",u16Length, g_u32WriteOffset);
					HandleDataPackage(szData, strlen((char *)szData));
				}

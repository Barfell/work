...
STATIC U32 g_u32Uart4RevCount = 0;

BOOL g_bStorage = FALSE;
...
STATIC VOID ReceiveUsartData(U8 type)
{
    ...
    switch(type)
    {
        ...
        case USART4_TYPE:	
			...
			g_u32Uart4RevCount = GetTickCount();
            break;
        default:
            break;
...
VOID ReadUsartData(USART_TypeDef* USARTx, U8 *pBuf, U16 *pLength)
{
	...
	while(DeleteQueue(pQueueInfo, pUdata, &Value) == TRUE)
	{
		*(pBuf + u16Count) = Value;
		u16Count++;	
	}
...
VOID USART1_Send(U8 *pBuf, U16 u16Count)
{
    U16 u16Cnt;
...
VOID USART2_Send(U8 *pBuf, U16 u16Count)
{
    U16 u16Cnt;
...
VOID USART3_Send(U8 *pBuf, U16 u16Count)
{
    U16 u16Cnt;
...
VOID UART4_Send(U8 *pBuf, U16 u16Count)
{
    U16 u16Cnt;

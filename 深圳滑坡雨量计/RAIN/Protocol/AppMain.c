...
//上传方式相关
extern unsigned char  AutoUploadFlag;
...
STATIC VOID HandleIndividualModule(S8 s8Value, PProtocolInfo pProtocolInfo)
{
	...
	read_dev_id(szId);
	sprintf((char *)szData, "#SCT%s=0x%s %s V:%f M%d=%f P%d= %f %s", szDeviceName, szId,get_time2(),get_dev_voltage(get_adc_value()), u8Value,GetFreq(u8Value), u8Value,GetTemperature(LTC2402_GetResistance(u8Value)), szEndInfo);
	HandleDataPackage(szData, strlen((char *)szData), pProtocolInfo);

	printf("EnterLowPowerMode_FLAG();\r\n");
	EnterLowPowerMode_FLAG();//进入到低功耗模式
...
STATIC U8 HandleAppCmdProc(U8 *pBuf, U16 u16Length, PProtocolInfo pProtocolInfo)
{
	...
	if((pBuf[0] == 'D') && (pBuf[1] == 'T'))
	{
		...
		else
		{
			...
			//szCmd2
			else if((memcmp(pBuf, szCmd2, ARRAY_SIZEOF(szCmd2)-4) == 0) )//&& (u16Length == ARRAY_SIZEOF(szCmd2)))
			{

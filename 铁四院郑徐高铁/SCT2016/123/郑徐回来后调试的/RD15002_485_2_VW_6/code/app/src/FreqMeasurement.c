...
void Time4_Config(void)	   
{
	...
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;							
	TIM_TimeBaseStructure.TIM_Period = 314;
	TIM_TimeBaseStructure.TIM_ClockDivision = 0;
	TIM_TimeBaseStructure.TIM_RepetitionCounter = 0;
	...
	TIM_OCInitStructure.TIM_Pulse = 265;
...
void FreqModuleInit(void)
{
	...
	TIM_TimeBaseStructure.TIM_Prescaler = 0;
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;							
	TIM_TimeBaseStructure.TIM_Period = 329;
	TIM_TimeBaseStructure.TIM_ClockDivision = 0;
	TIM_TimeBaseStructure.TIM_RepetitionCounter = 0;
	TIM_TimeBaseInit(TIM4, &TIM_TimeBaseStructure);
	TIM_SelectInputTrigger(TIM4,TIM_TS_ETRF);
...
{
	float vw1;
	unsigned int f = 0;			//频率范围
	unsigned char times = 0;	//延时长度
	
	SW_VW(ch);					//选择通道
	SW_VW_TRCN(0);				//tran 激励电路
	DelayMs(10);
		for(f = 500; f < 5000; f+=50) //200-9000hz
		{
			times = 1000*20 / f;
			TranPulse(f,20);	//发送20个脉冲
			if(times < 1)
				{DelayMs(1);}
			else
				{DelayMs(times);}
	...
	SW_VW_TRCN(1);						//recive 接收电路
	DelayMs(10);					//停顿，等待激励脉冲消除
	vw1 = Measure();
		
		//按照测量出的频率再次激励
		SW_VW_TRCN(0);				
		TranPulse( (unsigned int)vw1,30);
		times = 1000*30 / (unsigned int)vw1;
		DelayMs(times);
		SW_VW_TRCN(1);
		DelayMs(10);

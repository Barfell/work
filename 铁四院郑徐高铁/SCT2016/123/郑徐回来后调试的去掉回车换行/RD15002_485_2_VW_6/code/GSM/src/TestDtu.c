...
void HandleBoardInfo(U8 *pBuf)
{
	...
	U8 szData[50] = {0};
	ReadLockId(szId);
	sprintf((char *)szData, "#%s-ID=0x%x%x%x", g_szMachineType, szId[2], szId[1], szId[0]);
...
{
    U8 szData[50] = {0};
    sprintf((char *)szData, "#%s-Inner Voltage=%.3fV", g_szMachineType, get_dev_voltage(get_adc_value()));
		memcpy(pBuf, szData, strlen((char *)szData));
		memset(szData,0,50);
}
...
void HandleModeProc(U8 *pBuf)
{
		U8 szData[50] = {0};
		SCT_WakeUpTime = *(unsigned int*)0x0800c000;//读取flash的定时时间长度
		sprintf((char *)szData, "#%s-Interval Mode=%d sec", g_szMachineType, SCT_WakeUpTime);
		memcpy(pBuf, szData, strlen((char *)szData));
		memset(szData,0,50);
}
...
{
    U8 szData[50];
    sprintf((char *)szData, "#%s",get_time());
    memcpy(pBuf, szData, strlen((char *)szData));
	memset(szData,0,50);
}
...
void HandleTemperatureProc(U8 *pBuf)
{
	...
	for(u8Channel = 1; u8Channel < 9; u8Channel++)
	{
		...
		sprintf((char *)temp, "#%s-CH%d:T-%.3f",g_szMachineType, u8Channel, Temperature);
		Temperature = 0;
...
void HandleVWProc(U8 *pBuf)
{
	...
	memset(temp, 0, 60);
		...
		memset(temp, 0, 60);
		sprintf((char *)temp, "#%s-CH%d:F-%.3f T-%.3f",g_szMachineType,1, VM_Value, T_Value);
		VM_Value = 0;
		T_Value=0;
		strcat((char *)szData, (char *)temp);
		memset(temp,0,60);
	/*-----------------------------------*/
		...
		memset(temp, 0, 60);
		sprintf((char *)temp, "#%s-CH%d:F-%.3f T-%.3f",g_szMachineType,2, VM_Value, T_Value);
		VM_Value = 0;
		T_Value=0;
		strcat((char *)szData, (char *)temp);
		memset(temp,0,60);
	/*-----------------------------------*/
		...
		memset(temp, 0, 60);
		sprintf((char *)temp, "#%s-CH%d:F-%.3f T-%.3f",g_szMachineType,3, VM_Value, T_Value);
		VM_Value = 0;
		T_Value=0;
		strcat((char *)szData, (char *)temp);
		memset(temp,0,60);
	/*-----------------------------------*/
		...
		memset(temp, 0, 60);
		sprintf((char *)temp, "#%s-CH%d:F-%.3f T-%.3f",g_szMachineType,4, VM_Value, T_Value);
		VM_Value = 0;
		T_Value=0;
		strcat((char *)szData, (char *)temp);
		memset(temp,0,60);
	/*-----------------------------------*/
		...
		memset(temp, 0, 60);
		sprintf((char *)temp, "#%s-CH%d:F-%.3f T-%.3f",g_szMachineType,5, VM_Value, T_Value);
		VM_Value = 0;
		T_Value=0;
		strcat((char *)szData, (char *)temp);
		memset(temp,0,60);
	/*-----------------------------------*/
		...
		memset(temp, 0, 60);
		sprintf((char *)temp, "#%s-CH%d:F-%.3f T-%.3f",g_szMachineType,6, VM_Value, T_Value);
		VM_Value = 0;
		T_Value=0;
		strcat((char *)szData, (char *)temp);
		memset(temp,0,60);
	...
	/*-----------------------------------*/
		...
		memset(temp, 0, 60);
		sprintf((char *)temp, "#%s-CH%d:F-%.3f T-%.3f",g_szMachineType,8, VM_Value, T_Value);
		VM_Value = 0;
		T_Value=0;
		strcat((char *)szData, (char *)temp);
		memset(temp,0,60);
...
void Handle485Proc(U8 *pBuf)
{
	...
	sprintf((char *)ExTempStr, "#%s-CH%d:%s", (char *)g_szMachineType, 3, (char *)Temp485Str);
	...
	sprintf((char *)ExTempStr, "#%s-CH%d:%s", (char *)g_szMachineType, 4, (char *)Temp485Str);
	//printf("times:%d-485:%s\r\n",RetryTimes,ExTempStr);
	Temp485Strlength = 0;
	RetryTimes = 0;
	strcat((char *)RecivedData,(char *)ExTempStr);
	...
	sprintf((char *)ExTempStr, "#%s-CH%d:%s", (char *)g_szMachineType, 5, (char *)Temp485Str);
	//printf("times:%d-485:%s\r\n",RetryTimes,ExTempStr);
	Temp485Strlength = 0;
	RetryTimes = 0;
	strcat((char *)RecivedData,(char *)ExTempStr);
	...
	sprintf((char *)ExTempStr, "#%s-CH%d:%s", (char *)g_szMachineType, 6, (char *)Temp485Str);
	//printf("times:%d-485:%s\r\n",RetryTimes,ExTempStr);
	Temp485Strlength = 0;
	RetryTimes = 0;
	strcat((char *)RecivedData,(char *)ExTempStr);
	...
	sprintf((char *)ExTempStr, "#%s-CH%d:%s", (char *)g_szMachineType, 7, (char *)Temp485Str);
	//printf("times:%d-485:%s\r\n",RetryTimes,ExTempStr);
	Temp485Strlength = 0;
	RetryTimes = 0;
	strcat((char *)RecivedData,(char *)ExTempStr);
	...
	sprintf((char *)ExTempStr, "#%s-CH%d:%s", (char *)g_szMachineType, 8, (char *)Temp485Str);
	//printf("times:%d-485:%s\r\n",RetryTimes,ExTempStr);
	Temp485Strlength = 0;
	RetryTimes = 0;
	strcat((char *)RecivedData,(char *)ExTempStr);
	...
	sprintf((char *)ExTempStr, "#%s-CH%d:%s", (char *)g_szMachineType, 1, (char *)Temp485Str);
	//printf("times:%d-485:%s\r\n",RetryTimes,ExTempStr);
	Temp485Strlength = 0;
	RetryTimes = 0;
	strcat((char *)RecivedData,(char *)ExTempStr);
